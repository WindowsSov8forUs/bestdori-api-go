...
func (c *Chart) HasSpecialRhythm() bool {
	for _, note := range *c {
		if note.Type == NoteTypeDirectional {
			return true
		}
		if note.Type == NoteTypeLong || note.Type == NoteTypeSlide {
...
func (c *Chart) flatten() []Note {
	var notes []Note
	for _, note := range *c {
		if note.Type == NoteTypeSlide || note.Type == NoteTypeLong {
	...
	// 清理后跟 BPM 音符
	for len(notes) > 0 && notes[len(notes)-1].Type == NoteTypeBPM {
...
func (c *Chart) Stats() *ChartStats {
	...
	for _, note := range notes {
		if note.Type == NoteTypeBPM && note.BPM != 0.0 {
			bpmCount++
		}
	}
